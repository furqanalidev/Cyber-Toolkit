"""
brute_force_demo.py - Enhanced Educational Brute Force Demo

✅ Now includes:
- Password strength advisor
- Database logging of experiments
"""
...
from utils import password_strength, log_experiment  # NEW
...
class BruteForceDemoGUI(ctk.CTk):
    def __init__(self):
        ...
        self.lower_sw = ctk.CTkSwitch(inp_frame, text="lowercase"); self.lower_sw.select(); self.lower_sw.grid(row=0, column=1, padx=6, pady=6)
        self.upper_sw = ctk.CTkSwitch(inp_frame, text="UPPERCASE"); self.upper_sw.select(); self.upper_sw.grid(row=0, column=2, padx=6, pady=6)
        self.digit_sw = ctk.CTkSwitch(inp_frame, text="digits"); self.digit_sw.select(); self.digit_sw.grid(row=0, column=3, padx=6, pady=6)

        self.length_label = ctk.CTkLabel(inp_frame, text="Length:"); self.length_label.grid(row=1, column=0, padx=6, pady=6)
        self.length_entry = ctk.CTkEntry(inp_frame, width=120); self.length_entry.insert(0, "6"); self.length_entry.grid(row=1, column=1, padx=6, pady=6)

        self.attempts_label = ctk.CTkLabel(inp_frame, text="Attempts/sec:"); self.attempts_label.grid(row=1, column=2, padx=6, pady=6)
        self.attempts_entry = ctk.CTkEntry(inp_frame, width=160); self.attempts_entry.insert(0, "1000"); self.attempts_entry.grid(row=1, column=3, padx=6, pady=6)
        ...
        # --- Local Demo ---
        demo_frame = ctk.CTkFrame(self.scrollable_frame); demo_frame.pack(padx=20, pady=(0, 10), fill="x")
        ...
        # NEW: Check Strength Button
        self.strength_btn = ctk.CTkButton(demo_frame, text="Check Strength", command=self.check_password_strength)
        self.strength_btn.grid(row=1, column=2, padx=6, pady=6)
        ...
        self.footer = ctk.CTkLabel(self.scrollable_frame, text="Defensive: use long passphrases, slow hashes (bcrypt/argon2), rate-limits.", wraplength=700, justify="left")
    ...
    def check_password_strength(self):  # NEW
        pwd = self.password_entry.get().strip()
        if not pwd:
            self.append_result("Please enter a password to check strength.")
            return
        advice = password_strength(pwd)
        self.append_result("🔐 Password Strength Report:")
        self.append_result(advice)
        log_experiment("password_strength", f"pwd={pwd}", advice)

    def start_estimate(self):
        try:
            length = int(self.length_entry.get()); attempts = float(self.attempts_entry.get())
        except Exception:
            self.append_result("Invalid length or attempts/sec input"); return
        charset_size = 0
        if self.lower_sw.get(): charset_size += 26
        if self.upper_sw.get(): charset_size += 26
        if self.digit_sw.get(): charset_size += 10
        if charset_size == 0:
            self.append_result("Select at least one charset group"); return

        def _work():
            ks = estimate_keyspace(charset_size, length)
            secs = time_to_crack_seconds(ks, attempts); days = secs / 86400
            ...
            log_experiment("keyspace_estimate", f"length={length}, attempts={attempts}", f"days={days:.2f}")
        threading.Thread(target=_work, daemon=True).start()

    def start_demo(self):
        pwd = self.password_entry.get(); salt = self.salt_entry.get() or ""
        ...
        def _work():
            ...
            log_experiment("brute_force_demo", f"pwd={pwd}, candidates={candidates}", f"found={found}, elapsed={elapsed:.4f}s")
        threading.Thread(target=_work, daemon=True).start()
